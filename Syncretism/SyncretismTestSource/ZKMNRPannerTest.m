...
- (void)testPannerCofficientsInQuadLayout
{
	...
	[quadLayout release];
	XCTAssertTrue([quadLayout retainCount] == 1, @"The panner should retain the speaker layout (RC: %u)", [quadLayout retainCount]);
	
	ZKMNRPannerSource* source = [[ZKMNRPannerSource alloc] init];
	[panner registerPannerSource: source];
	[source release];
	
	XCTAssertTrue([source retainCount] == 1, @"The panner should retain the source (RC: %u)", [source retainCount]);
	XCTAssertTrue([source numberOfMixerCoefficients] == 4, @"A panner position on a quadraphonic layout should have 4 coeffs");
	...
	XCTAssertEqualWithAccuracy(mixerCoeffs[0], 0.f, 0.01, @"LR coeff should be 0.");
	XCTAssertEqualWithAccuracy(mixerCoeffs[1], 0.7f, 0.01, @"LF coeff should be 0.7");
	XCTAssertEqualWithAccuracy(mixerCoeffs[2], 0.7f, 0.01, @"RF coeff should be 0.7");
	XCTAssertEqualWithAccuracy(mixerCoeffs[3], 0.f, 0.01, @"RR coeff should be 0.");
		
	XCTAssertEqualWithAccuracy([source pannerGain], 1.f, 0.001, @"Panning should not change the gain");
		
	point.azimuth = -1.f;
	[source setCenter: point];
	// directly behind should have mixer coeffs of [LR LF RF RR] == [0.7, 0.0, 0.0, 0.7]
	mixerCoeffs = [source mixerCoefficients];
	XCTAssertEqualWithAccuracy(mixerCoeffs[0], 0.7f, 0.01, @"LR coeff should be 0.7");
	XCTAssertEqualWithAccuracy(mixerCoeffs[1], 0.f, 0.01, @"LF coeff should be 0.");
	XCTAssertEqualWithAccuracy(mixerCoeffs[2], 0.f, 0.01, @"RF coeff should be 0.");
	XCTAssertEqualWithAccuracy(mixerCoeffs[3], 0.7f, 0.01, @"RR coeff should be 0.7");
		
	ZKMNRSphericalCoordinateSpan span = { 2.f, 0.f };
	[source setCenter: point span: span gain: 1.f];
	// all the speakers should be used here
	mixerCoeffs = [source mixerCoefficients];
	XCTAssertTrue(mixerCoeffs[0] > 0.f, @"LR coeff should be > 0");
	XCTAssertTrue(mixerCoeffs[1] > 0.f, @"LF coeff should be > 0");
	XCTAssertTrue(mixerCoeffs[2] > 0.f, @"RF coeff should be > 0");
	XCTAssertTrue(mixerCoeffs[3] > 0.f, @"RR coeff should be > 0");
	...
	[panner release];
	XCTAssertTrue([quadLayout retainCount] == 1, @"The panner should release the speaker layout when released (RC: %u)", [quadLayout retainCount]);
	XCTAssertTrue([source retainCount] == 1, @"The panner should release the source when released (RC: %u)", [source retainCount]);
...
- (void)testPannerCofficientsInSphereLayout
{
	...
	XCTAssertTrue([source numberOfMixerCoefficients] == 12, @"A panner position on a sphere layout should have 12 coeffs");
	...
	XCTAssertEqual(nonZeroCoeffCount, (NSUInteger) 2, @"There should be exactly 2 non-zero coefficients.");
	XCTAssertEqualWithAccuracy([source pannerGain], 1.f, 0.001, @"Panning should not change the gain");
	...
	nonZeroCoeffCount = [self countNonZeroMixerCoeffs: source];
	XCTAssertEqual(nonZeroCoeffCount, (NSUInteger) 4, @"There should be exactly 4 non-zero coefficients.");
	XCTAssertEqualWithAccuracy([source pannerGain], 1.f, 0.001, @"Panning should not change the gain");
	...
	nonZeroCoeffCount = [self countNonZeroMixerCoeffs: source];
	XCTAssertEqual(nonZeroCoeffCount, (NSUInteger) 7, @"There should be exactly 7 non-zero coefficients.");
	XCTAssertEqualWithAccuracy([source pannerGain], 1.f, 0.001, @"Panning should not change the gain");
...
- (void)testPannerCofficientsInBottomHemisphereDomeLayout
{
	...
	XCTAssertTrue([source numberOfMixerCoefficients] == 8, @"A panner position on a bottom hemisphere dome should have 8 coeffs");
	...
	nonZeroCoeffCount = [self countNonZeroMixerCoeffs: source];
	XCTAssertEqual(nonZeroCoeffCount, (NSUInteger) 0, @"There should be no non-zero coefficients.");
	...
	nonZeroCoeffCount = [self countNonZeroMixerCoeffs: source];
	XCTAssertEqual(nonZeroCoeffCount, (NSUInteger) 3, @"There should be exactly 3 non-zero coefficients.");
	XCTAssertEqualWithAccuracy([source pannerGain], 1.f, 0.001, @"Panning should not change the gain");
...
- (void)testSpeakerFindingInQuadLayout
{
	...
	XCTAssertNotNil(closestSpeaker, @"The speaker closest to point { 0.25f, 0.f, 1.f } is nil");
	
	ZKMNRSphericalCoordinate speakerPos = [closestSpeaker coordPlatonic];
	XCTAssertEqualWithAccuracy(speakerPos.azimuth, 0.25f, 0.01, @"Speaker position should be { 0.25f, 0.f, 1.f }");
	XCTAssertEqualWithAccuracy(speakerPos.zenith, 0.f, 0.01, @"Speaker position should be { 0.25f, 0.f, 1.f }");
	XCTAssertEqualWithAccuracy(speakerPos.radius, 1.f, 0.01, @"Speaker position should be { 0.25f, 0.f, 1.f }");
	
		// { 0.8f, 0.f, 1.f }
	point.azimuth = 0.8f;
	closestSpeaker = [panner speakerClosestToPoint: point];
	XCTAssertNotNil(closestSpeaker, @"The speaker closest to point { 0.8f, 0.f, 1.f } is nil");
	
	speakerPos = [closestSpeaker coordPlatonic];
	XCTAssertEqualWithAccuracy(speakerPos.azimuth, 0.75f, 0.01, @"Speaker position should be { 0.75f, 0.f, 1.f }");
	XCTAssertEqualWithAccuracy(speakerPos.zenith, 0.f, 0.01, @"Speaker position should be { 0.75f, 0.f, 1.f }");
	XCTAssertEqualWithAccuracy(speakerPos.radius, 1.f, 0.01, @"Speaker position should be { 0.75f, 0.f, 1.f }");
...
- (void)testSpeakerFindingInSphereLayout
{
	...
	ZKMNRSphericalCoordinate speakerPos = [closestSpeaker coordPlatonic];
	XCTAssertEqualWithAccuracy(speakerPos.azimuth, 0.25f, 0.01, @"Speaker position should be { 0.25f, 0.f, 1.f }");
	XCTAssertEqualWithAccuracy(speakerPos.zenith, 0.0f, 0.01, @"Speaker position should be { 0.25f, 0.19f, 1.f }");
	XCTAssertEqualWithAccuracy(speakerPos.radius, 1.f, 0.01, @"Speaker position should be { 0.25f, 0.f, 1.f }");
		// { 0.8f, 0.f, 1.f }
	point.azimuth = 0.8f;
	closestSpeaker = [panner speakerClosestToPoint: point];
	...
	speakerPos = [closestSpeaker coordPlatonic];
	XCTAssertEqualWithAccuracy(speakerPos.azimuth, 0.75f, 0.01, @"Speaker position should be { 0.75f, 0.f, 1.f }");
	XCTAssertEqualWithAccuracy(speakerPos.zenith, 0.0f, 0.01, @"Speaker position should be { 0.75f, 0.19f, 1.f }");
	XCTAssertEqualWithAccuracy(speakerPos.radius, 1.f, 0.01, @"Speaker position should be { 0.75f, 0.f, 1.f }");
	...
	point.zenith = 0.5f;
	closestSpeaker = [panner speakerClosestToPoint: point];
	XCTAssertNotNil(closestSpeaker, @"The speaker closest to point { 0.25f, 0.5f, 1.f } is nil");
	
		// { 0.75f, -0.5f, 1.f };
	point.azimuth = 0.75f;
	point.zenith = -0.5f;
	closestSpeaker = [panner speakerClosestToPoint: point];
	XCTAssertNotNil(closestSpeaker, @"The speaker closest to point { 0.75f, -0.5f, 1.f } is nil");
	
		// { 0.25f, 0.2f, 1.f };
	point.azimuth = 0.25f;
	point.zenith = 0.2f;
	closestSpeaker = [panner speakerClosestToPoint: point];
	XCTAssertNotNil(closestSpeaker, @"The speaker closest to point { 0.25f, 0.2f, 1.f } is nil");
	
	speakerPos = [closestSpeaker coordPlatonic];
	XCTAssertEqualWithAccuracy(speakerPos.azimuth, 0.25f, 0.01, @"Speaker position should be { 0.25f, 0.19f, 1.f }");
	XCTAssertEqualWithAccuracy(speakerPos.zenith, 0.19f, 0.01, @"Speaker position should be { 0.25f, 0.19f, 1.f }");
	XCTAssertEqualWithAccuracy(speakerPos.radius, 1.f, 0.01, @"Speaker position should be { 0.25f, 0.19f, 1.f }");
	
		// { 0.75f, -0.2f, 1.f };
	point.azimuth = 0.75f;
	point.zenith = -0.2f;
	closestSpeaker = [panner speakerClosestToPoint: point];
	XCTAssertNotNil(closestSpeaker, @"The speaker closest to point { 0.75f, -0.2f, 1.f } is nil");
	
	speakerPos = [closestSpeaker coordPlatonic];
	XCTAssertEqualWithAccuracy(speakerPos.azimuth, 0.75f, 0.01, @"Speaker position should be { 0.75f, -0.19f, 1.f }");
	XCTAssertEqualWithAccuracy(speakerPos.zenith, -0.19f, 0.01, @"Speaker position should be { 0.75f, -0.19f, 1.f }");
	XCTAssertEqualWithAccuracy(speakerPos.radius, 1.f, 0.01, @"Speaker position should be { 0.75f, -0.19f, 1.f }");		
