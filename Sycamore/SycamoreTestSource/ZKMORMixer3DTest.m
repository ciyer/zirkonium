...
- (void)testMixer3D
{
	...
	XCTAssertNoThrow([mixer3D setMaxFramesPerSlice: 2048], @"Set max frames per slice should not cause an error");
	...
	XCTAssertNoThrow([mixer3D initialize], @"Initialize should not throw an exception");


	XCTAssertNoThrow([mixer3D uninitialize], @"Uninitialize should not throw an exception");
}
